...
import { ChevronLeft, Lock, Ban, UserX, FileArchive, ChevronDown, Sparkles } from "lucide-react";
import { Card, CardHeader } from "@/components/ui/Card";
import { SeverityBadge, StatusBadge } from "@/components/ui/Badge";
import { Button } from "@/components/ui/Button";
import { useToast } from "@/components/ui/Toast";
import { AttackGraphCanvas } from "@/components/graph/AttackGraphCanvas";
import { incidentGraph } from "@/lib/mock-data";
import type { Incident } from "@/types";

const TABS = ["Overview", "Attack Path", "Timeline", "AI Investigation", "MITRE ATT&CK", "Evidence"] as const;

const actionIcon: Record<string, typeof Lock> = {
  "Isolate Device": Lock,
  "Block IP": Ban,
  "Disable Account": UserX,
  "Quarantine File": FileArchive,
  "Restrict Egress": Ban,
  "Create Ticket": FileArchive,
  "Notify Data Owner": FileArchive,
  "Force Password Reset": UserX,
};

export function IncidentDetailClient({ incident }: { incident: Incident }) {
    ...
    <div className="flex flex-col gap-4">
      ...
      {tab === "Overview" && (
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            ...
            <div className="p-4">
              <AttackGraphCanvas nodes={incidentGraph.nodes} edges={incidentGraph.edges} height={220} />
            </div>
          </Card>
          <div className="flex flex-col gap-5">
            <Card>
              <CardHeader title="AI Summary" action={<Sparkles className="h-4 w-4 text-primary" />} />
              <p className="p-5 pt-3 text-sm leading-relaxed text-text-2">{incident.aiSummary}</p>
            </Card>
            <Card>
      ...
      {tab === "Attack Path" && (
        <Card>
          ...
          <div className="p-4">
            <AttackGraphCanvas nodes={incidentGraph.nodes} edges={incidentGraph.edges} height={280} />
          </div>
        </Card>
      )}
      ...
      {tab === "Evidence" && (
        <Card>
          ...
          <div className="p-5 pt-4 text-sm text-text-2">
            Forensic artifacts, packet captures and endpoint snapshots collected during this investigation are
            preserved for chain-of-custody review.
          </div>
        </Card>
      )}
    </div>
